...
import { setRecommendationRefresh } from "../redux/contentSlice";

const CreateVideo = () => {
  const navigate = useNavigate();
  const { channelData } = useSelector((state) => state.user);
  const { allChannelData } = useSelector((state) => state.user);
  ...
  const handlePublish = async () => {
    ...
    try {
      const result = await axios.post(
        `${serverUrl}/api/content/upload-video`,
        formData,
        {
      ...
      dispatch(setRecommendationRefresh());
      ...
      const updatedAllChannels = allChannelData.map((ch) =>
        ch._id === channelData._id
          ? {
              ...ch,
              videos: [...(ch.videos || []), result.data.video],
            }
          : ch,
      );
