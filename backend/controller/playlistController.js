  ...
  try {
    ...
    if (!title || !channelId) {
      return res.status(400).json({
        success: false,
        message: "Playlist title and channel ID are required",
      });
    }

    // 1️⃣ Check channel exists
    const channel = await Channel.findById(channelId);
    if (!channel) {
    ...
    // 2️⃣ Validate videos belong to channel
    if (videoIds.length > 0) {
      ...
      if (videos.length !== videoIds.length) {
        return res.status(400).json({
          success: false,
          message: "Some videos do not belong to this channel",
        });
      }
    }
    ...
    // 4️⃣ Push playlist to channel
    await Channel.findByIdAndUpdate(channelId, {
      $push: { playlists: newPlaylist._id },
    });

    // 5️⃣ Fetch populated playlist (IMPORTANT)
