...
export const toggleSaveVideo = async (req, res) => {
  try {
    ...
    await video.save();
    console.log("Toggled save for video:", videoId, "by user:", userId);
   return res.status(200).json(video);
  } catch (error) {
...
//     }
//     console.log("Fetched saved videos for user:", userId, savedVideos);
...
export const getSavedVideos = async (req, res) => {
  try {
    ...
    let savedVideos = await Video.find({ saveBy: userId })
      .populate("channel", "name avatar")
      .lean(); // 🚀 faster & cleaner objects

    // 🧹 Remove null / broken populated data
    savedVideos = savedVideos.filter(video => video && video.channel);

    return res.status(200).json({
      success: true,
      count: savedVideos.length,
      savedVideos, // ✅ ALWAYS an array
    });
    ...
    return res.status(500).json({
      success: false,
      message: "Server error while fetching saved videos",
      savedVideos: [], // ✅ frontend never breaks
    });
  }
};
