...
export const getSavedShorts = async (req, res) => {
  try {
    ...
    let savedShorts = await Short.find({ saveBy: userId })
      .populate("channel", "name avatar")
      .lean(); // 🚀 better performance & clean objects

    // 🧹 Remove broken / null populated data
    savedShorts = savedShorts.filter(short => short && short.channel);

    return res.status(200).json({
      success: true,
      count: savedShorts.length,
      savedShorts, // ✅ ALWAYS array
    });
