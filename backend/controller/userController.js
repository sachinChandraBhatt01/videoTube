...
export const getRecommendedContent = async (req, res) => {
  try {
    ...
    /* =====================================================
       1️⃣ COLLECT KEYWORDS (history + liked + saved)
    ===================================================== */
    const historyKeywords = user.history.map(h => h.contentId?.title || "");

    const likedVideos = await Video.find({ likes: userId }).select("title");
    const likedShorts = await Short.find({ likes: userId }).select("title");
    const savedVideos = await Video.find({ saveBy: userId }).select("title");
    const savedShorts = await Short.find({ saveBy: userId }).select("title");
    ...
    /* =====================================================
       2️⃣ ALWAYS FETCH LATEST (FRESH) CONTENT
    ===================================================== */
    ...
    const freshShorts = await Short.find()
      .sort({ createdAt: -1 })
      .limit(10)
      .populate("channel", "name avatar");

    /* =====================================================
       3️⃣ BUILD KEYWORD CONDITIONS (ONLY IF KEYWORDS EXIST)
    ===================================================== */
    ...
    const uniqueById = (arr) =>
      arr.filter(
        (v, i, self) =>
          i === self.findIndex(x => x._id.toString() === v._id.toString())
      );

    const finalRecommendedVideos = uniqueById([
      ...freshVideos,
      ...recommendedVideos,
    ]);

    const finalRecommendedShorts = uniqueById([
      ...freshShorts,
      ...recommendedShorts,
    ]);

    /* =====================================================
       5️⃣ REMAINING CONTENT (EXCLUDE RECOMMENDED)
    ===================================================== */
    const recommendedVideoIds = finalRecommendedVideos.map(v => v._id);
    const recommendedShortIds = finalRecommendedShorts.map(s => s._id);
    ...
    ===================================================== */
    return res.status(200).json({
      recommendedVideos: finalRecommendedVideos,
      recommendedShorts: finalRecommendedShorts,
      remainingVideos,
      remainingShorts,
      usedKeywords: allKeywords,
    });
    ...
    return res
      .status(500)
      .json({ message: `Recommendation failed: ${error.message}` });
  }
};
